      ...
      >
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl">
          ...
          <div className="mb-5">
            ...
            {/* 3-user grid: 2 on top, 1 centered below */}
              ...
              {users.length > 2 && (
                <div className="flex justify-center">
                  <div className="w-[calc(50%-5px)]">
                    <UserCard
                      u={users[2]}
                      selected={selectedUserId === users[2].id}
                      onSelect={() => { setSelectedUserId(users[2].id); setError('') }}
                    />
                  </div>
                </div>
              )}
...
function UserCard({
  ...
  u: { id: string; name: string; role: string; avatar: string; color: string }
  selected: boolean
  onSelect: () => void
}) {
  return (
    <button
      onClick={onSelect}
      className={`
        relative p-4 rounded-2xl border transition-all duration-200 text-left group active:scale-[0.97] w-full
        ${selected
    ...
    >
      ...
      <p className={`text-sm font-bold leading-tight transition-colors ${selected ? 'text-white' : 'text-white/70 group-hover:text-white'}`}>
        {u.name}
      </p>
      <p className={`text-[10px] font-medium mt-0.5 leading-tight transition-colors ${selected ? 'text-brand' : 'text-white/30 group-hover:text-white/50'}`}>
        {u.role}
      </p>
    </button>
  )
